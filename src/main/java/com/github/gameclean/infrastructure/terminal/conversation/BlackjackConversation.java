 ...
 * swept.
 *
 * <p>Its affordance carries no tokens: the whole between-interaction state rides as the opaque envelope
 * ({@link Affordance#getPayload()}), which this handler alone narrows back to the {@link BlackjackRound} the
 * arming presenter parked — the cast is the {@code SelectCommand} cast's twin, the one place the shell's
 * generic plumbing meets this dialogue's concrete currency. Like every conversation it pulls a <em>fresh</em>
...
public class BlackjackConversation implements Conversation {
    ...
    public SelectionKind kind() {
        return SelectionKind.BLACKJACK;
    }
    ...
    public void resume(Command command, Affordance affordance) {
        ...
        BlackjackRound round = (BlackjackRound) affordance.getPayload();
        switch (command) {
