 ...
 * attempts. The driving adapter (the seed reader) parses the authoring syntax — a {@code "scn2, scn3"}
 * list into {@code scenes}, a {@code "12/50"} fraction into {@code chanceNumerator}/{@code chanceDenominator}
 * — and this carrier holds the result, possibly <em>domain</em>-invalid (e.g. a zero denominator, a
 * malformed scene id) so the use-case gate can reject it as a presented outcome. The
 * {@link com.github.gameclean.core.model.item.Chance} / {@link com.github.gameclean.core.model.item.SpawnRule}
 * value objects are constructed inside {@link InitializeGameUseCase}.
 *
