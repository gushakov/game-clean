 ...
 * items lying in that scene and the NPCs standing in it (neither is part of the shared prologue — they belong
 * to the scene each use case presents, which for {@code look} is the current one) and presents the scene with
 * them. On a
    ...
    public void playerLooksAround() {
        try {
            ...
            presenter.presentScene(scene,
                    itemOps.findItemsInScene(scene.getId()), npcOps.findNpcsInScene(scene.getId()));
